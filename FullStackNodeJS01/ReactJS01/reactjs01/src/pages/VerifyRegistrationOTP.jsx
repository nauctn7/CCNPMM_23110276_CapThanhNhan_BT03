...
const VerifyRegistrationOTP = () => {
    ...
    const handleVerify = async (e) => {
        ...
        try {
            const response = await api.post('/api/verify-registration-otp', { email, otp });
            ...
            setError(error.response?.data?.EM || 'Xác thực thất bại');
        }
        setLoading(false);
    };
    ...
    const handleResendOTP = async () => {
        ...
        setLoading(true);
        try {
            const response = await api.post('/api/resend-registration-otp', { email });
            if (response.data.EC === 0) {
                setCountdown(60);
        ...
        <div className="min-h-screen bg-gradient-to-r from-pink-100 to-pink-200 flex items-center justify-center py-12 px-4">
            <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl p-8">
                <div className="text-center mb-8">
                    <h2 className="text-3xl font-bold text-pink-500">Xác thực tài khoản</h2>
